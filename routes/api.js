...
require('../settings.js');
const totalRequest = require('../lib/request.js');
...
router.get('/ai/chatgpt', async (req, res, next) => {
    ...
    try {
        ...
        const allowedEmails = ['bayuardana213@gmail.com', 'yuuxdrestapi@gmail.com'];
        ...
        await checkLimit(apikey);
        let requestRecord = await totalRequest.findOne();
        if (!requestRecord) {
          requestRecord = await totalRequest.create({ count: 0 });
        }
...
router.get('/ai/deepai', async (req, res, next) => {
    ...
    try {
        ...
        // Menghitung dan menyimpan jumlah permintaan ke MongoDB
        let requestRecord = await totalRequest.findOne();
        if (!requestRecord) {
          requestRecord = await totalRequest.create({ count: 0 });
        }
